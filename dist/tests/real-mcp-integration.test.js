...
import { createMCPClientManager } from '../external/MCPClientManager';
            ...
            for (const fallback of fallbackTests) {
                ...
                try {
                    ...
                    if (BrokerClass) {
                        ...
                        const methods = Object.getOwnPropertyNames(Object.getPrototypeOf(broker)).filter(name => typeof broker[name] === 'function' && name !== 'constructor');
    ...
    describe('Production Readiness Assessment', () => {
        it('should assess if the system is production-ready with current integration', async () => {
            ...
            if (!isProductionReady) {
                ...
                const missingCritical = criteria.requiredRealServers.filter(key => !status.servers.find(s => s.key === key && s.isReal));
...
//# sourceMappingURL=real-mcp-integration.test.js.map