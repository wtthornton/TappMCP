...
export class ContextPersistenceEngine {
    config;
    memoryStorage;
    memoryIndex; // field -> value -> entry IDs
    compressionCache;
    constructor(config = {}) {
     ...
     */
    async getSessionContext(sessionId, limit = 50) {
        return this.search({
            sessionId,
            limit,
            sortBy: 'timestamp',
    ...
    async getStatistics() {
        ...
        const stats = {
            ...
            averageTokensPerEntry: totalEntries > 0 ? memoryEntries.reduce((sum, e) => sum + e.tokens, 0) / totalEntries : 0,
            storageSize: {
    ...
    async calculateDiskSize() {
        // Implementation would calculate disk usage
        return 0;
    }
    async archiveToDisk(entry) {
...
export function createContextPersistenceEngine(config) {
    return new ContextPersistenceEngine(config);
}
//# sourceMappingURL=ContextPersistenceEngine.js.map