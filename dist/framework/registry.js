 ...
 */
import { MCPToolFactory } from './mcp-tool';
import { MCPResourceFactory } from './mcp-resource';
import { MCPPromptFactory } from './mcp-prompt';
export class MCPRegistry {
    ...
    async getStats() {
        ...
        const resourceHealthChecks = await Promise.allSettled(resources.map(resource => resource.healthCheck()));
        const promptHealthChecks = await Promise.allSettled(prompts.map(prompt => prompt.healthCheck()));
        const healthyTools = toolHealthChecks.filter(result => result.status === 'fulfilled' && result.value).length;
        const healthyResources = resourceHealthChecks.filter(result => result.status === 'fulfilled' && result.value).length;
        const healthyPrompts = promptHealthChecks.filter(result => result.status === 'fulfilled' && result.value).length;
