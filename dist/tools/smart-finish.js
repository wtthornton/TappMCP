...
export async function handleSmartFinish(input) {
    ...
    try {
        ...
        const projectAnalysis = await simpleAnalyzer.runBasicAnalysis(projectPath, validatedInput.validationLevel === 'enterprise'
            ? 'deep'
        ...
        const qualityScorecard = scorecardGenerator.generateScorecard(securityResult, staticResult, coverageMetrics, performanceMetrics, {
        ...
        const response = {
            ...
            projectAnalysisReport: {
                ...
                security: {
                    score: projectAnalysis.security.summary.total > 0
                        ? Math.max(0, 100 -
                            projectAnalysis.security.summary.critical * 20 -
                            projectAnalysis.security.summary.high * 10)
                        : 100,
...
function generateComprehensiveValidation(qualityScorecard, validationLevel, role, processCompliance) {
    const qualityGates = [
        {
            name: 'Test Coverage',
            status: qualityScorecard.coverage.grade === 'A' || qualityScorecard.coverage.grade === 'B'
                ? 'pass'
                : 'fail',
        ...
        {
            name: 'Security Score',
            status: qualityScorecard.security.grade === 'A' || qualityScorecard.security.grade === 'B'
                ? 'pass'
                : 'fail',
        ...
        },
        {
            ...
            status: qualityScorecard.complexity.grade === 'A' || qualityScorecard.complexity.grade === 'B'
                ? 'pass'
                : 'fail',
        ...
        },
        {
            ...
            status: qualityScorecard.performance.grade === 'A' || qualityScorecard.performance.grade === 'B'
                ? 'pass'
                : 'fail',
...
function generateRoleSpecificRecommendations(role, qualityScorecard) {
    ...
    if (role === 'developer') {
        ...
        if (qualityScorecard?.performance.grade === 'A' ||
 ...
 */
async function calculateRealTestCoverage(projectAnalysis, codeIds) {
