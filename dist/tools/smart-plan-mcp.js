 ...
 */
export class SmartPlanMCPTool extends MCPTool {
    constructor() {
        super(config);
    }
    /**
     ...
     */
    generatePlanId(projectId, planType) {
     ...
     */
    generatePhases(planType, _scope, externalMCPs, _qualityRequirements) {
        const phases = [];
        switch (planType) {
            case 'development':
                phases.push(this.createPhase('Planning and Setup', 'Project planning, requirements gathering, and initial setup', 1, [
                ...
                break;
            case 'testing':
                phases.push(this.createPhase('Test Planning', 'Plan and design comprehensive testing strategy', 1, [
                ...
                break;
            case 'deployment':
                phases.push(this.createPhase('Deployment Planning', 'Plan deployment strategy and prepare environments', 1, [
                ...
                break;
            case 'maintenance':
                phases.push(this.createPhase('Maintenance Planning', 'Plan ongoing maintenance activities', 1, [
                ...
                break;
            case 'migration':
                phases.push(this.createPhase('Migration Planning', 'Plan migration strategy and prepare for transition', 2, [
        ...
        if (externalMCPs && externalMCPs.length > 0) {
            const integrationPhase = this.createPhase('External Integration', 'Integrate with external MCPs and services', 2, externalMCPs.map(mcp => this.createTask(`Integrate ${mcp.name}`, `Integrate with ${mcp.description}`, mcp.estimatedEffort, [], [`${mcp.name} Integration`, 'API Documentation'])), [
     ...
     */
    createPhase(name, description, duration, tasks, milestones) {
        return {
            name,
            description,
            duration,
     ...
     * Create a task
     */
    createTask(name, description, effort, dependencies, deliverables, assignee) {
        return {
            name,
            description,
            effort,
     ...
     * Create a milestone
     */
    createMilestone(name, description, dueDate, successCriteria) {
        return {
            name,
            description,
            dueDate,
     ...
     */
    generateRisks(_phaseName, duration) {
        const risks = [
            {
     ...
     */
    calculateTimeline(phases, timelineInput) {
     ...
     * Allocate resources
     */
    allocateResources(resourcesInput, _duration, _businessContext) {
     ...
     */
    generateTeam(size, _businessContext) {
     ...
     * Define quality requirements
     */
    defineQualityRequirements(qualityRequirements, _planType) {
     ...
     * Calculate business value
     */
    calculateBusinessValue(input, projectPlan) {
     ...
     * Generate next steps
     */
    generateNextSteps(input, _projectPlan) {
        const steps = [
     ...
     * Generate recommendations
     */
    generateRecommendations(input, _projectPlan) {
...
//# sourceMappingURL=smart-plan-mcp.js.map