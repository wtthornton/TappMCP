...
import { SmartWriteMCPTool } from './smart-write-mcp.js';
describe('SmartWriteMCPTool', () => {
    ...
    describe('Business Context', () => {
        it('should calculate business value based on priority', async () => {
            ...
            expect(highResult.data?.businessValue.timeSaved).toBeGreaterThan(lowResult.data?.businessValue.timeSaved ?? 0);
            expect(highResult.data?.businessValue.costPrevention).toBeGreaterThan(lowResult.data?.businessValue.costPrevention ?? 0);
        });
        it('should adjust business value based on code type', async () => {
            ...
            expect(apiResult.data?.businessValue.timeSaved).toBeGreaterThan(functionResult.data?.businessValue.timeSaved ?? 0);
        });
    });
    ...
    describe('Test Cases Generation', () => {
        it('should generate test cases based on coverage requirements', async () => {
            ...
            expect(highResult.success).toBe(true);
            expect(lowResult.success).toBe(true);
            expect(highResult.data?.generatedCode.testCases.length).toBeGreaterThan(lowResult.data?.generatedCode.testCases.length ?? 0);
        });
    });
});
//# sourceMappingURL=smart-write-mcp.test.js.map