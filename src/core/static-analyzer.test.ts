...
describe('StaticAnalyzer', () => {
  ...
  describe('analyzeComplexity', () => {
    it('should analyze complexity and flag high complexity', () => {
      const analyzer = new StaticAnalyzer('/test');
      const analyzeComplexity = (analyzer as any).analyzeComplexity.bind(analyzer);
      ...
      expect(result.some((issue: { id: string }) => issue.id === 'high-complexity')).toBe(true);
    });
...
${Array(60).fill('  console.log("test");').join('\n')}
      ...
      expect(result.some((issue: { id: string }) => issue.id === 'long-function')).toBe(true);
    });
  });
