...
describe('SecurityScanner - REAL TESTS', () => {
  ...
  describe('runSecurityScan - ACTUAL SECURITY SCANNING', () => {
    it('should complete security scan in reasonable time', async () => {
      ...
      console.log(
        `Real scan took: ${duration.toFixed(2)}ms, found ${result.vulnerabilities.length} vulnerabilities`
      );
    }, 35000);
    ...
    it('should build correct npm audit command', () => {
      ...
      expect((scanner as any).projectPath).toBe('/test/path');
    });
  ...
  describe('Severity Mapping - ACTUAL LOGIC TESTING', () => {
    it('should map npm audit severities correctly', () => {
      ...
      const mapNpmSeverity = (scanner as any).mapNpmSeverity.bind(scanner);
      ...
      expect(mapNpmSeverity(undefined as any)).toBe('moderate');
    });

    it('should map retire.js severities correctly', () => {
      const scanner = new SecurityScanner('/test');
      const mapRetireSeverity = (scanner as any).mapRetireSeverity.bind(scanner);
    ...
    it('should map OSV severity scores correctly', () => {
      const scanner = new SecurityScanner('/test');
      const mapOSVSeverity = (scanner as any).mapOSVSeverity.bind(scanner);
  ...
  describe('File Discovery - REAL FILE SYSTEM OPERATIONS', () => {
    it('should find actual source files in project', () => {
      ...
      expect(files.some((f: string) => f.endsWith('utils.js'))).toBe(true);
      expect(files.some((f: string) => f.endsWith('config.json'))).toBe(true);
      expect(files.some((f: string) => f.endsWith('package.json'))).toBe(true); // Created in beforeEach

      // Should NOT find markdown files
      expect(files.some((f: string) => f.endsWith('README.md'))).toBe(false);

      // All results should be absolute paths (Windows: C:\ or Unix: /)
      expect(files.every((f: string) => f.includes(':') || f.startsWith('/'))).toBe(true);
    });
    ...
    it('should handle subdirectories correctly', () => {
      ...
      expect(files.some((f: string) => f.includes('src') && f.endsWith('main.ts'))).toBe(true);
      expect(files.some((f: string) => f.includes('tests') && f.endsWith('test.js'))).toBe(true);
  ...
  describe('INTEGRATION - Full Security Scan with Real Files', () => {
    it('should scan real project and provide comprehensive results', async () => {
      ...
      // File with secrets
      writeFileSync(
        join(testProjectPath, 'src', 'config.ts'),
        `
      ...
      writeFileSync(
        join(testProjectPath, 'src', 'unsafe.ts'),
        `
      ...
      // Safe file
      writeFileSync(
        join(testProjectPath, 'src', 'safe.ts'),
        `
      ...
      console.log(
        `Comprehensive scan found ${result.vulnerabilities.length} issues in ${result.scanTime}ms`
      );
