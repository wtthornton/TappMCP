...
export class ProjectScanner {
  ...
  async scanProject(
    ...
    try {
      ...
      const qualityIssues = await this.analyzeQualityIssues(
        projectPath,
        projectStructure,
        detectedTechStack
      );
  ...
  private async scanProjectStructure(
    projectPath: string,
    analysisDepth: 'quick' | 'standard' | 'deep'
  ): Promise<ProjectAnalysis['projectStructure']> {
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            if (
              ['node_modules', '.git', 'dist', 'build', 'coverage', '.next', '.nuxt'].includes(
                entry.name
              )
            ) {
              continue;
  ...
  private async analyzeQualityIssues(
    ...
    if (
      detectedTechStack.includes('typescript') &&
  ...
  private async extractProjectMetadata(
    projectPath: string
  ): Promise<ProjectAnalysis['projectMetadata']> {
    const packageJsonPath = path.join(projectPath, 'package.json');
