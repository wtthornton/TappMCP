...
describe('BusinessAnalyzer - REAL TESTS', () => {
  ...
  describe('analyzeRequirements - ACTUAL FUNCTIONALITY', () => {
    it('should extract SPECIFIC goals from e-commerce request', () => {
      const request =
        'We want to create an e-commerce platform with payment processing and inventory management';
      ...
      expect(
        result.riskFactors.some(risk => risk.includes('payment') || risk.includes('security'))
      ).toBe(true);
    });
    ...
    it('should extract DIFFERENT goals for different domains', () => {
      ...
      expect(healthResult.primaryGoals.some(goal => goal.toLowerCase().includes('patient'))).toBe(
        true
      );

      // Finance should have finance-specific goals
      expect(
        financeResult.primaryGoals.some(
          goal => goal.toLowerCase().includes('trading') || goal.toLowerCase().includes('stock')
        )
      ).toBe(true);

      // Constraints should be domain-specific
      expect(healthResult.constraints.some(c => c.includes('HIPAA') || c.includes('privacy'))).toBe(
        true
      );

      expect(
        financeResult.constraints.some(c => c.includes('regulatory') || c.includes('compliance'))
      ).toBe(true);
    });
    ...
    it('should NOT return generic goals for specific requests', () => {
      ...
      expect(
        result.primaryGoals.some(goal => goal.includes('real-time') || goal.includes('chat'))
      ).toBe(true);

      expect(
        result.primaryGoals.some(goal => goal.includes('encryption') || goal.includes('security'))
      ).toBe(true);
    });

    it('should complete analysis in ACTUAL time, not just <100ms check', () => {
      const complexRequest =
        'Build a comprehensive enterprise resource planning system with financial management, human resources, supply chain, manufacturing, and customer relationship management modules integrated with AI-powered analytics and blockchain-based audit trails';
  ...
  describe('assessComplexity - ACTUAL COMPLEXITY CALCULATION', () => {
    it('should calculate complexity based on REAL factors', () => {
      ...
      const complexRequest =
        'Build distributed microservices with Kubernetes orchestration, service mesh, event streaming, and multi-region deployment';
  ...
  describe('generateUserStories - ACTUAL STORY GENERATION', () => {
    it('should generate stories that match the requirements', () => {
      ...
      const adminAuthStory = result.find(
        s => s.asA === 'Admin' && s.title.includes('authentication')
      );
      ...
      const userProfileStory = result.find(
        s => s.asA === 'Regular User' && s.title.includes('Profile')
      );
  ...
  describe('INTEGRATION - Full workflow validation', () => {
    it('should maintain consistency across all methods', () => {
      ...
      expect(
        requirements.primaryGoals.some(
          g => g.toLowerCase().includes('banking') || g.toLowerCase().includes('financial')
        )
      ).toBe(true);

      expect(
        requirements.constraints.some(
          c => c.toLowerCase().includes('security') || c.toLowerCase().includes('compliance')
        )
      ).toBe(true);
      ...
      expect(
        stories.some(
          s =>
            s.title.toLowerCase().includes('biometric') ||
