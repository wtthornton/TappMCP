...
describe('BusinessAnalyzer', () => {
  ...
  describe('analyzeRequirements', () => {
    it('should extract primary goals from business request', () => {
      const request =
        'We want to create a user management system to handle authentication and user profiles';
    ...
    it('should handle complex business requests with multiple goals', () => {
      const request =
        'Need to implement an e-commerce platform with payment processing, inventory management, and customer support features for online retailers';
  ...
  describe('identifyStakeholders', () => {
    it('should identify stakeholders from business request', () => {
      const request =
        'Create a system for managers to track employee performance and for developers to maintain code quality';

      const result = businessAnalyzer.identifyStakeholders(request);

      expect(result.length).toBeGreaterThan(0);
      expect(result.some((s: { role: string }) => s.role === 'Manager')).toBe(true);
      expect(result.some((s: { role: string }) => s.role === 'Developer')).toBe(true);
    });
    ...
    it('should assign appropriate influence and interest levels', () => {
      ...
      result.forEach((stakeholder: { influence: string; interest: string }) => {
  ...
  describe('assessComplexity', () => {
    ...
    it('should assess high complexity for complex requests', () => {
      const request =
        'Build a real-time scalable API with multiple external integrations, advanced security, and compliance requirements';
    ...
    it('should identify technical complexity factors', () => {
      ...
      expect(result.factors.some((factor: string) => factor.includes('Real-time'))).toBe(true);
      expect(result.factors.some((factor: string) => factor.includes('API'))).toBe(true);
      expect(result.factors.some((factor: string) => factor.includes('Security'))).toBe(true);
    });
  });
  ...
  describe('identifyRisks', () => {
    it('should identify timeline risks from urgent requests', () => {
      ...
      expect(result.some((risk: { id: string }) => risk.id === 'timeline-pressure')).toBe(true);
    });
    ...
    it('should identify scope creep risks from complex requests', () => {
      ...
      expect(result.some((risk: { id: string }) => risk.id === 'scope-creep')).toBe(true);
    });
    ...
    it('should identify technical risks from new technology mentions', () => {
      ...
      expect(result.some((risk: { id: string }) => risk.id === 'technical-risk')).toBe(true);
    });
    ...
    it('should assign appropriate risk levels', () => {
      ...
      result.forEach((risk: { probability: string; impact: string; severity: string }) => {
  ...
  describe('generateUserStories', () => {
    ...
    it('should include quality-focused user story', () => {
      ...
      expect(
        result.some((story: { title: string }) => story.title === 'Ensure System Quality')
      ).toBe(true);
    });
    ...
    it('should assign appropriate priorities and effort estimates', () => {
      ...
      result.forEach((story: { priority: string; estimatedEffort: number }) => {
  ...
  describe('performance', () => {
    it('should maintain performance targets across all methods', () => {
      const request =
        'Complex business request with multiple goals, stakeholders, and requirements for performance testing';
