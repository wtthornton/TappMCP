...
export class WebSearchBroker {
  private config: WebSearchBrokerConfig;
  ...
  private getFallbackSearchResults(query: string, maxResults: number): SearchResult[] {
  ...
  private getFallbackTrends(topic: string): Trend[] {
    return [
      {
  ...
  private getFallbackValidation(assumption: string): ValidationResult {
    return {
  ...
  private getFallbackMarketAnalysis(domain: string): MarketAnalysis {
    return {
