...
import {
  ...
  ContextEntrySchema,
  type ContextEntry,
} from './DeepContext7Broker';

describe('DeepContext7Broker - Week 2 Enhanced', () => {
  let broker: DeepContext7Broker;
  ...
  describe('Context Storage and Retrieval', () => {
    it('should store and retrieve context entries', async () => {
      const contextEntry: Partial<ContextEntry> = {
      ...
      const relevantContexts = await broker.getRelevantContext(
        'test_session_001',
    ...
    it('should apply compression when enabled', async () => {
      const longContent = 'This is a very long context entry that should be compressed. '.repeat(
        50
      );
      ...
      expect(contexts[0].compressionInfo!.compressedLength).toBeLessThan(
        contexts[0].compressionInfo!.originalLength
      );
    });
  ...
  describe('Context Suggestions', () => {
    ...
    it('should generate relevant context suggestions', async () => {
      const suggestions = await broker.generateContextSuggestions(
        'suggest_session',
        'smart_begin',
  ...
  describe('Context Injection', () => {
    ...
    it('should inject context adaptively', async () => {
      const result = await broker.injectContext(
        'inject_session',
        'smart_write',
    ...
    it('should apply minimal injection mode', async () => {
      const result = await broker.injectContext(
        'inject_session',
        'smart_write',
        'Simple component creation task',
        'minimal'
      );
    ...
    it('should track injection metadata correctly', async () => {
      const result = await broker.injectContext(
        'inject_session',
        'smart_write',
        'Test injection metadata tracking'
      );
  ...
  describe('Context Cleanup and Management', () => {
    ...
    it('should preserve high-value contexts during cleanup', async () => {
      ...
      const remainingContexts = await broker.getRelevantContext(
        'cleanup_session',
        'high-relevance',
        'smart_begin'
      );
  ...
  describe('Advanced Context Features', () => {
    ...
    it('should validate context entry schema', () => {
      const validEntry = {
        ...
        toolName: 'smart_begin',
        contextType: 'user_query' as const,
    ...
    it('should handle different persistence levels', async () => {
      ...
      const crossSessionResults = await broker.getRelevantContext(
        'different_session',
        'project context',
        'smart_plan',
        { crossSession: true }
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle large context content', async () => {
      ...
      const results = await broker.getRelevantContext(
        'large_session',
        'large content',
        'smart_orchestrate'
      );
...
describe('Context Entry Schema Validation', () => {
  ...
  it('should validate optional fields with defaults', () => {
    const minimalEntry = {
      ...
      sessionId: 'session_002',
      toolName: 'smart_write',
      contextType: 'tool_output' as const,
