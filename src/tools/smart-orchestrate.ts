...
function generateNextSteps(
  ...
  if (orchestrationValidation?.recommendations?.length > 0) {
    orchestrationValidation.recommendations.forEach((recommendation: string) => {
      nextSteps.push({
        step: recommendation,
        role: 'developer',
...
export async function handleSmartOrchestrate(input: unknown): Promise<{
  ...
  try {
    ...
    const orchestrationValidation = generateOrchestrationValidation(
      workflowResult,
      validatedInput.orchestrationLevel,
      validatedInput.role,
...
function generateOrchestrationValidation(
  workflowResult: any,
  ...
  orchestrationLevel: string;
  roleSpecificOrchestration: boolean;
  workflowValidation: Array<{
    phase: string;
