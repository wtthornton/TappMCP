...
vi.mock('../core/plan-generator.js', () => ({
  PlanGenerator: vi.fn().mockImplementation(() => ({
    generatePlan: vi.fn().mockImplementation(async () => {
      ...
      userStories: [
        { id: 'US001', title: 'User Registration', description: 'As a user, I want to register for an account' },
        { id: 'US002', title: 'User Login', description: 'As a user, I want to log into my account' },
      ],
      businessValue: {
        estimatedROI: 150,
        timeToMarket: 6,
      ...
      risks: [
        { id: 'R001', description: 'Security vulnerabilities', impact: 'high', probability: 'medium' },
...
describe('SmartPlanEnhancedMCPTool', () => {
  ...
  describe('Deliverables', () => {
    it('should generate comprehensive deliverables', async () => {
      ...
      expect(result.data?.deliverables.successMetrics[0]).toContain('User registration completion rate');
  ...
  describe('Data Structure', () => {
    it('should maintain backward compatibility with data object', async () => {
      ...
      expect(result.data?.data.planType).toBe('comprehensive');
