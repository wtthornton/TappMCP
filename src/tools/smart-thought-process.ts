  ...
  try {
    ...
    const response = {
      ...
      summary: {
        ...
        requirementsMet: thoughtProcess.step4_validation.requirementsCheck.filter(check =>
          check.includes('✅')
        ).length,
        totalRequirements: thoughtProcess.step4_validation.requirementsCheck.length,
        qualityScore:
          (Object.values(thoughtProcess.step4_validation.qualityMetrics).filter(
            metric => metric === 'Excellent'
          ).length /
