...
const SmartFinishInputSchema = z.object({
  ...
  codeIds: z.array(z.string()).optional().default([]),
...
function validateTestCoverage(codeIds: string[], requiredCoverage: number): {
  score: number;
...
function validateSecurityScore(codeIds: string[], requiredScore: number): {
  score: number;
  status: 'pass' | 'fail' | 'warning';
  details: string;
} {
...
function validateComplexityScore(codeIds: string[], requiredScore: number): {
  score: number;
  status: 'pass' | 'fail' | 'warning';
  details: string;
} {
...
function validateMaintainabilityScore(codeIds: string[], requiredScore: number): {
  score: number;
  status: 'pass' | 'fail' | 'warning';
  details: string;
} {
...
function validateBusinessRequirements(
  ...
  costPrevention: { score: number; status: 'pass' | 'fail'; details: string };
  timeSaved: { score: number; status: 'pass' | 'fail'; details: string };
  userSatisfaction: { score: number; status: 'pass' | 'fail'; details: string };
} {
...
export async function handleSmartFinish(input: unknown): Promise<{
  ...
  try {
    ...
    if (testCoverage.status === 'fail') recommendations.push('Increase test coverage to meet quality requirements');
    if (securityScore.status === 'fail') recommendations.push('Address security vulnerabilities');
    if (complexityScore.status === 'fail') recommendations.push('Reduce code complexity');
    if (maintainabilityScore.status === 'fail') recommendations.push('Improve code maintainability');
    if (businessValidation.costPrevention.status === 'fail') recommendations.push('Increase cost prevention measures');
    if (businessValidation.timeSaved.status === 'fail') recommendations.push('Optimize for time savings');
    if (businessValidation.userSatisfaction.status === 'fail') recommendations.push('Improve user experience');
    if (productionValidation.securityScan.status === 'fail') recommendations.push('Fix security issues before deployment');
    if (productionValidation.performanceTest.status === 'fail') recommendations.push('Optimize performance before deployment');
    if (productionValidation.documentationComplete.status === 'fail') recommendations.push('Complete documentation before deployment');
    if (productionValidation.deploymentReady.status === 'fail') recommendations.push('Complete deployment configuration');
