...
export class ErrorPreventionSystem {
  ...
  private patternIndex: Map<string, Set<string>>; // category -> pattern IDs
  private learningData: Map<
    string,
    {
      triggers: string[];
      outcomes: Array<{ result: 'prevented' | 'occurred' | 'false_positive'; timestamp: Date }>;
    }
  >;
   ...
   */
  async analyzeForErrors(context: AnalysisContext): Promise<ErrorPrediction[]> {
  ...
  getStatistics(): PreventionStats {
    ...
    for (const analysis of this.analysisHistory) {
      ...
      for (const prediction of analysis.predictions) {
        ...
        if (learningData) {
          const preventedOutcomes = learningData.outcomes.filter(
            o => o.result === 'prevented'
          ).length;
          const falsePositiveOutcomes = learningData.outcomes.filter(
            o => o.result === 'false_positive'
          ).length;
    ...
    const accuracyRate =
      totalPredictions > 0 ? ((totalPredictions - falsePositives) / totalPredictions) * 100 : 0;
  ...
  private initializeBuiltInPatterns(): void {
    const builtInPatterns: Partial<ErrorPattern>[] = [
      {
      ...
      },
      {
        ...
        category: 'memory',
        pattern:
          '(fs\\.createReadStream|new\\s+EventEmitter|setInterval)(?![^}]*(?:\\.close\\(\\)|\\.destroy\\(\\)|clearInterval))',
   ...
   */
  private findSimilarPastErrors(patternId: string): Array<{
    pattern: string;
    resolution: string;
    effectiveness: number;
  }> {
   ...
   * Helper methods
   */
  private indexPattern(pattern: ErrorPattern): void {
  ...
  private recordAnalysis(context: AnalysisContext, predictions: ErrorPrediction[]): void {
  ...
  private findMatchingPatterns(errorMessage: string, stack?: string): ErrorPattern[] {
    const matching: ErrorPattern[] = [];
    const searchText = `${errorMessage} ${stack || ''}`.toLowerCase();

    for (const pattern of this.errorPatterns.values()) {
      if (
        searchText.includes(pattern.name.toLowerCase()) ||
  ...
  private async createPatternFromError(error: {
    message: string;
    stack?: string;
    context: AnalysisContext;
  }): Promise<void> {
  ...
  private categorizeError(errorMessage: string): ErrorPattern['category'] {
  ...
  private updatePreventionStrategies(pattern: ErrorPattern, resolution: string): void {
    // Add new strategy based on resolution if not already present
    const existingStrategy = pattern.preventionStrategies.find(s =>
      s.description.toLowerCase().includes(resolution.toLowerCase().substring(0, 20))
    );
  ...
  private calculatePreventionRate(patternId: string): number {
  ...
  private calculatePatternPriority(pattern: ErrorPattern): number {
  ...
  private estimateEffort(strategy: string): 'low' | 'medium' | 'high' {
...
export function createErrorPreventionSystem(
  config?: Partial<PreventionConfig>
