...
export class BackendAnalyzer extends BaseCategoryIntelligenceEngine {
  ...
  async analyzeCode(
    ...
    const performance = await this.performanceAnalyzer.analyzePerformance(code, technology, insights);
    const security = await this.apiSecurityAnalyzer.analyzeSecurity(code, technology, insights);
    const scalability = await this.scalabilityAnalyzer.analyzeScalability(code, technology, insights);
...
func fetchData() (map[string]interface{}, error) {
  ...
  private generateGenericBackendCode(description: string, technology: string, role?: string): string {
    return `// ${description}
// Generated by BackendAnalyzer
