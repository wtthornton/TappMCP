...
import { PerformanceTracker, PerformanceMetrics } from './PerformanceTracker.js';
...
export class ToolChainOptimizer {
  ...
  constructor(config: Partial<OptimizerConfig> = {}) {
    ...
    this.executionEngine = new ExecutionEngine(
      this.toolRegistry,
      (toolName, executionTime, cost, success) =>
        this.performanceTracker.updatePerformanceProfile(toolName, executionTime, cost, success)
    );
  ...
  async createOptimizedPlan(planDefinition: {
    ...
    description?: string;
    tools: Array<{
      tool?: string; // Support test format with 'tool' property
      name?: string; // Support 'name' property for backward compatibility
    ...
    const plan: ExecutionPlan = {
      ...
      constraints: {
        maxExecutionTime:
          planDefinition.constraints?.maxExecutionTime ||
          this.estimateTotalExecutionTime(finalSteps),
        maxCost: planDefinition.constraints?.maxCost || this.estimateTotalCost(finalSteps),
  ...
  suggestOptimizations(plan: ExecutionPlan): Array<{
    ...
    const parallelizableSteps = this.performanceTracker.findParallelizableSteps(
      plan.steps,
      this.toolRegistry
    );
    if (parallelizableSteps.length > 0) {
      const timeSavings = this.performanceTracker.estimateParallelizationSavings(
        parallelizableSteps,
        this.toolRegistry
      );
      suggestions.push({
        type: 'parallelism' as const,
    ...
    const expensiveSteps = this.performanceTracker.findExpensiveSteps(
      plan.steps,
      this.toolRegistry
    );
    ...
    const unreliableSteps = this.performanceTracker.findUnreliableSteps(
      plan.steps,
      this.toolRegistry
    );
