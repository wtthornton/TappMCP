...
describe('MCP Protocol Simulation', () => {
  it('should handle smart tool calls directly', async () => {
    ...
    expect((result.data as any).projectId).toBeDefined();
    console.log(
      `✅ smart_begin direct call successful, projectId: ${(result.data as any).projectId}`
    );
  });
  ...
  it('should chain smart tool calls with context', async () => {
    ...
    const projectId = (beginResult.data as any).projectId;
    ...
    expect((planResult.data as any).projectId).toBe(projectId);
    console.log(
      `✅ Step 2 - smart_plan: ${(planResult.data as any).projectPlan.phases.length} phases planned`
    );
    ...
    console.log('smart_write response projectId:', (writeResult.data as any).projectId);
    console.log('Expected projectId:', projectId);
    expect((writeResult.data as any).projectId).toBe(projectId);
    const codeId = (writeResult.data as any).codeId;
    ...
    expect((finishResult.data as any).projectId).toBe(projectId);
    console.log(
      `✅ Step 4 - smart_finish: ${(finishResult.data as any).qualityScorecard.overall.score}% quality`
    );
